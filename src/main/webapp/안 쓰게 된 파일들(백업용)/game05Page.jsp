...
<script>
  ...
  function chooseOne(){
	console.log(dislikeList[i]);
    left_select.style.backgroundImage = "url('" + "images/싫어하는 사람 유형/" + dislikeList[i] +  "')";
    right_select.style.backgroundImage = "url('" + "images/싫어하는 사람 유형/" + dislikeList[i+1] +  "')";
  ...
  function initRound() {
    if(dislikeList.length == 1) {
      document.querySelector("#result_select").style.backgroundImage = "url('" + "images/싫어하는 사람 유형/" + dislikeList[0] +  "')";

      battleRound.innerText = "최종 선택";
      document.querySelector(".result_selection").style.display = "block";
      result_text.style.display = "block";
      result_text.innerText = "#" + nameList[dislikeListOrigin.indexOf(dislikeList[0])];

      document.querySelector("#left").style.display = "none";
      document.querySelector("#right").style.display = "none";
      document.querySelector("#verse").style.display = "none";

      setCookie("result01", dislikeListOrigin.indexOf(dislikeList[0]) + 1 , 1);
      
      setTimeout(function() {location.href = "index.jsp";} , 2000);
      return;
    }

    if(dislikeList.length == 2) { document.querySelector("#battleRound").innerText = "결승전"; }
    else document.querySelector("#battleRound").innerText = dislikeList.length +"강";
  ...
  left_select.addEventListener('click', function(){
    ...
    if(i < dislikeList.length - 1) chooseOne();
    else {
      initRound();
    }
  });
